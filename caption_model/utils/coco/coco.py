...
class COCO:
    ...
    def filter_by_cap_len(self, max_cap_len):
        ...
        keep_ann = {}
        keep_img = {}
        for ann in tqdm(self.dataset):
            if len(word_tokenize(ann['caption']))<=max_cap_len:
                keep_ann[ann['image_id']] = keep_ann.get(ann['image_id'], 0) + 1
                keep_img[ann['image_id']] = keep_img.get(ann['image_id'], 0) + 1

        # self.dataset['annotations'] = \
        #     [ann for ann in self.dataset if keep_ann.get(ann['image_id'],0) > 0]
        # self.dataset['images'] = \
        #     [img for img in self.dataset['images'] if keep_img.get(img['id'],0)>0]
